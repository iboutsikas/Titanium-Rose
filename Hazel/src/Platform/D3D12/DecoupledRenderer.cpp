...
namespace Hazel
{
    DECLARE_SHADER("SurfaceShader-Decoupled");
    ...
    void DecoupledRenderer::ImplRenderVirtualTextures()
    {
        ...
        while (counter < s_DecoupledObjects.size())
        {
            ...
            for (int i = 0; i < MaxItemsPerQueue && counter < s_DecoupledObjects.size(); i++)
            {
                ...
                PIXBeginEvent(cmdlist.Get(), PIX_COLOR(200, 0, 0), "Virtual Pass: %s", tex->GetIdentifier());
    ...
    {
        return ShaderLibrary->GetAs<D3D12Shader>(ShaderName);
    }
    ...
    void DecoupledRenderer::ImplOnInit()
    {
        ...
        auto shader = Hazel::CreateRef<D3D12Shader>(ShaderPath, pipelineStateStream);
    ...
    void DecoupledRenderer::ImplSubmit(D3D12ResourceBatch& batch, Ref<GameObject>& gameObject)
    {
        ...
        if (gameObject->DecoupledComponent.VirtualTexture == nullptr)
        {
            ...
            opts.Width = width;
            opts.Height = height;
            opts.MipLevels = albedo->GetMipLevels();
            opts.Format = albedo->GetFormat();
